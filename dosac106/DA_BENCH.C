...
int main()
{
	...
	/* TITLE */
	printf("### DosAlloc [ Ver:%.2f ]  - Speed Benchmark Test -  ###\n",
													DosAlloc_VERSION );
	#if defined(_MSC_VER)
	...
	for( iLap=0, i=MAX_TEST-1 ; 0<=i ; i-- ){
		...
		for( j=0 ; j<MAX_POINTERS ; j++ ){
			if( LPV[ k = Random(MAX_POINTERS) ] ){
				...
				if( k%3==0 )      LPV[k] = (LPVOID)!0;
				else if( k%3==1 ) LPV[k] = (LPVOID)!0;
				else              LPV[k] = (LPVOID)!0;
				assert( LPV[k] == (LPVOID)!0 );
			}
		}
	...
	for( iLap=0, i=MAX_TEST-1 ; 0<=i ; i-- ){
		...
		for( j=0 ; j<MAX_POINTERS ; j++ ){
			if( LPV[ k = Random(MAX_POINTERS) ] != NULL ){
				...
				if( k%3==0 )      LPV[k] = (LPVOID)_fmalloc( ALLOC1 );
				else if( k%3==1 ) LPV[k] = (LPVOID)_fmalloc( ALLOC2 );
				else              LPV[k] = (LPVOID)_fmalloc( ALLOC3 );
				assert( LPV[k] != NULL );
			}
		}
		if( i % (MAX_TEST/RAP_TIMES) == 0 ){
	...
	for( iLap=0, i=MAX_TEST-1 ; 0<=i ; i-- ){
		...
		for( j=0 ; j<MAX_POINTERS ; j++ ){
			if( LPV[ k = Random(MAX_POINTERS) ] != NULL ){
				...
				if( k%3==0 )      LPV[k] = _dAlloc( ALLOC1 );
				else if( k%3==1 ) LPV[k] = _dAlloc( ALLOC2 );
				else              LPV[k] = _dAlloc( ALLOC3 );
				assert( LPV[k] != NULL );
			}
		}
		if( i % (MAX_TEST/RAP_TIMES) == 0 ){
