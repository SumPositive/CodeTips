...
#define ALLOC_MIN   sizeof(WORD) /* 最少アロケートサイズ */
#define ALLOC_MAX     512        /* 最大アロケートサイズ */
#define ALLOC_STEP     16        /* サイズ・ステップ */
...
wBench2( HWND hWnd )
{
	...
	// TITLE
	sprintf(z100, "WinAlloc: Benchmark - RANDOM SIZE TEST -  [ Ver:%.2f ]",
												WinAlloc_VERSION );
	wputs_title(0, z100 );
	...
	for( iLap=0, i=MAX_TEST-1 ; 0<=i ; i-- )
	{
		...
		for( j=0 ; j<MAX_POINTER ; j++ )
		{
			if( LPointers[ k = Random(MAX_POINTER) ] )
			{
				...
				LPointers[k] = (LPVOID)1;
				if( LPointers[k] == NULL ) Error("Empty loop");
			}
		}
	...
	for( iLap=0, i=MAX_TEST-1 ; 0<=i ; i-- )
	{
		...
		for( j=0 ; j<MAX_POINTER ; j++ )
		{
			if( LPointers[ k = Random(MAX_POINTER) ] )
			{
				...
				w = (w / ALLOC_STEP) * ALLOC_STEP;
				if( w<ALLOC_MIN ) w = ALLOC_MIN;
				LPointers[k] = (LPVOID)MAKELONG( 0, GlobalAlloc( GPTR, w ) );
